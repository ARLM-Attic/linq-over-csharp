...
namespace SampleSupport
{
    public class SampleHarness : IEnumerable<Sample>
    {
        ...
        public SampleHarness()
        {
            ...
            MethodInfo[] methods = samplesType.GetMethods(BindingFlags.Public|BindingFlags.Instance|BindingFlags.DeclaredOnly);
        ...
        {
            int blockStart = allCode.IndexOf(blockName);
            
            if (blockStart == -1)
                return "// " + blockName + " code not found";
            blockStart = allCode.LastIndexOf(Environment.NewLine, blockStart);
            if (blockStart == -1)
                blockStart = 0;
            else
