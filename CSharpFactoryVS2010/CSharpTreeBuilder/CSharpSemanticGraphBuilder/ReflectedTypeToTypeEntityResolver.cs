...
namespace CSharpTreeBuilder.CSharpSemanticGraphBuilder
{
  ...
  public sealed class ReflectedTypeToTypeEntityResolver : MetadataResolver<TypeEntity, System.Type>
  {
    ...
    // ----------------------------------------------------------------------------------------------
    private ReflectedTypeToTypeEntityResolver(ReflectedTypeToTypeEntityResolver template, TypeParameterMap typeParameterMap)
    ...
    {
      return new ReflectedTypeToTypeEntityResolver(this, typeParameterMap);
    }
    ...
    public override TypeEntity Target
    {
      get
      {
        // If this resolver is a clone, then it's target is the same as the template's resolution state.
        if (IsGenericClone)
        {
          return (OriginalGenericTemplate as ReflectedTypeToTypeEntityResolver)._Target;
        }
    ...
    public override ResolutionState ResolutionState
    {
      get
      {
        ...
        if (IsGenericClone)
        {
          return (OriginalGenericTemplate as ReflectedTypeToTypeEntityResolver)._ResolutionState;
        }
